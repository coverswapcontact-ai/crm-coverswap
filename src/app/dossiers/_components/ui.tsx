...
export function PastilleEtape({ etape, libelle }: { etape: EtapeDossier; libelle: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border-[0.5px] border-[#2A2D34] bg-[#16181D] px-2 py-0.5 text-[11px] whitespace-nowrap text-[#D1D5DB]">
      <span aria-hidden className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: COULEURS_ETAPE[etape] }} />
      {libelle}
    </span>
  );
}
