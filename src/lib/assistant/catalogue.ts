...
export const CATALOGUE: OutilQuelconque[] = [...OUTILS_LECTURE, outilPointDuJour, ...OUTILS_ANALYSE, ...OUTILS_ECRITURE];
...
export type OutilVue = { nom: string; titre: string; description: string; niveau: NiveauOutil; libelleNiveau: string; famille: "LECTURE" | "ANALYSE" | "ECRITURE" };

/** La liste lisible, pour l'écran Paramètres et le rapport. */
export function catalogueVue(): OutilVue[] {
  const famille = (o: OutilQuelconque): OutilVue["famille"] => (OUTILS_ANALYSE.includes(o) ? "ANALYSE" : o.niveau === "LECTURE" ? "LECTURE" : "ECRITURE");
