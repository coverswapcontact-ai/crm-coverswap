...
export async function calculerPointDuJour(maintenant: Date = new Date(), options: { depuis?: Date | null; memoriser?: boolean } = {}) {
  ...
  const [leads, simulationsEspace, simulationsSite, demandesDevis, accords, paiements, projetsClients, changements, mails, pilotage, rappels, actionsDossiers, campagne, sante, messagesNonLus, propositionsEnAttente, messagesRecents] = await Promise.all([
    ...
    prisma.accordDevis.findMany({ where: { createdAt: { gte: depuis }, retireLe: null }, select: { id: true, totalHt: true, numeroDevis: true, nomSignataire: true, createdAt: true, dossier: { select: { id: true, clientNom: true } } }, take: 50 }),
  ...
  const point = {
    ...
    nouveautes: {
      ...
      demandesDevis,
      devisSignes: accords.map((a) => ({ dossierId: a.dossier.id, client: a.dossier.clientNom, numero: a.numeroDevis, montant: a.totalHt, le: a.createdAt.toISOString() })),
